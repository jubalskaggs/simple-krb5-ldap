...
@CreateDS( 
  ...
  partitions =
  {
      @CreatePartition(
          name = "vcontext",
          suffix = "dc=vcontext,dc=local",
          contextEntry = @ContextEntry( 
              entryLdif =
                  "dn: dc=vcontext,dc=local\n" +
                  "dc: vcontext\n" +
                  "objectClass: top\n" +
                  "objectClass: domain\n\n" ),
          indexes = 
...
@CreateLdapServer (
        ...
        saslHost="localhost",
        saslPrincipal="ldap/localhost@VCONTEXT.LOCAL",
        saslMechanisms =
        {
...
@CreateKdcServer(primaryRealm = "VCONTEXT.LOCAL",
  kdcPrincipal = "krbtgt/VCONTEXT.LOCAL@VCONTEXT.LOCAL",
  searchBaseDn = "dc=vcontext,dc=local",
  transports = 
  { 
...
public class KerberosSetup {
    ...
    protected void startKDC(final String[] args) throws Exception {
        ...
        createLdapServer.setSaslPrincipal("ldap/" + canonicalHost + "@VCONTEXT.LOCAL");
        fixTransportAddress(createLdapServer, canonicalHost);
        ldapServer = ServerAnnotationProcessor.instantiateLdapServer(createLdapServer, directoryService);
        ldapServer.setSearchBaseDn("dc=vcontext,dc=local");
        ldapServer.start();
    }
